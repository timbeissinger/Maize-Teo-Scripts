...
### Downsample
downsamp <- function(x) sum(sample(c(1,0),size=13,replace=T,prob=c(x,1-x)))

downed <- sapply(mafsMaize$knownEM,downsamp) # do downsampling

mafsMaize$downSampled <- downed
...
### Write results
write.table(allChromosomes,file="tF_maize_downsampled.txt")
save(allChromosomes,file="tF_maize_downsampled.Robj")
